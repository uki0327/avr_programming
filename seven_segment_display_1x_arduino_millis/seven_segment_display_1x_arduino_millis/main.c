...
// 타이머0의 분주비를 64로 설정한 경우 오버플로 인터럽트가 발생하기 까지의 마이크로초 경과시간
...
// 오버플로 인터럽트가 발생한 경우 밀리초 단위 증가를 제외하고 남는 마이크로 초 단위 증가시간
...
unsigned long millis() {
	...
	// timer0_millis 값을 읽는 동안 timer0_millis 값이 변하지 않도록 인터럽트 비활성화
	cli();
